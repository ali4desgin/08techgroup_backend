...
class ContentController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		if($request->method()=="POST"){
			...
			if($job=="header"){
				$header_string_edit =
					 file_get_contents("json/header.json");
				...
				file_put_contents("json/header.json",$newJsonString);
				
				$editDone = "تم تعديل بيانات القائمة العلوية بنجاح";
			}else if($job=="footer"){
				$footer_string_edit =
					 file_get_contents("json/footer.json");
				...
				$aboutus_string_edit =
					 file_get_contents("json/aboutus.json");
				...
				file_put_contents("json/aboutus.json",$newJsonString);
		...
		$header_string = file_get_contents("json/header.json");
		$header = json_decode($header_string,true);
		
		
		$footer_string = file_get_contents("json/footer.json");
